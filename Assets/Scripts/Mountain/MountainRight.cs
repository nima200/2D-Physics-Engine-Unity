...
public class MountainRight : MonoBehaviour
{
    ...
    private Mesh GenerateBaseMesh()
    {
        ...
        // 1) Vertex in middle of left segment
        // 2) Vertex in middle of right segment
        ...
        // Base mesh vertices.
        _vertices[0] = new Vector3(0f, 0f); // Bottom left vertex
        _vertices[1] = new Vector3(_width / 2, 0f); // Bottom right vertex
        ...
        _triangles[0] = 0;
        _triangles[1] = _vertexCount / 2;
        _triangles[2] = 1;

        // Setting up right triangle
        // Based on the bottom right, bottom left, and midpoint vertex
        _triangles[_triangleCount/2] = 0;
        _triangles[_triangleCount/2 + 1] = _vertexCount - 1;
        _triangles[_triangleCount/2 + 2] = _vertexCount / 2;
    ...
    private void GenerateRecursiveMesh(int vertexCount, int triangleCount, int currentTriangleIndex, int leftVertexIndex, int recursionLevel)
    {
        while (true)
        {
            ...
            _triangles[currentTriangleIndex + 1] = leftVertexIndex + 1;
            _triangles[currentTriangleIndex + 2] = leftVertexIndex;

            // New right triangle
            _triangles[currentTriangleIndex + 3] = 0;
            _triangles[currentTriangleIndex + 4] = leftVertexIndex + 2;
            _triangles[currentTriangleIndex + 5] = leftVertexIndex + 1;
